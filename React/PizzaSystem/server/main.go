...
type Pizza struct {
	...
	Crust string             `json:"crust" bson:"crust"`
}
...
func createPizza(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to add the pizza"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch the pizza"})
		return
	}
